// client.js — client with pass/respond UI and priority info
...
function renderState(state){
  ...
  turnDiv.innerHTML = `<strong>Turn:</strong> ${state.turn.number} <strong>Player:</strong> ${state.turn.currentPlayerId} <strong>Phase:</strong> ${state.turn.phase}`;

  // Priority info
  const prioDiv = document.getElementById('priority');
  prioDiv.innerHTML = `<strong>Priority open:</strong> ${state.priority && state.priority.active ? 'YES' : 'NO'} — <strong>Stack depth:</strong> ${state.stackDepth}`;
  ...
  // Pass button (during priority)
  const passBtn = document.createElement('button'); passBtn.innerText = 'Pass'; passBtn.onclick = ()=> ws.send(JSON.stringify({ type:'intent', intent: { type:'pass' } })); controls.appendChild(passBtn);
